...
const Footer = () => {
  ...
  // [] SERVICES
  const lstServices = [
    { id: 1, title: "Chính sách bảo mật thông tin", url: "/policy-security" },
  ...
  const lstContact = [
    {
      id: 1,
      title:
        "⚛️ Địa chỉ: 63-65 đường số 10, Phường An Lợi Đông, Quận 2, Thành Phố Thủ Đức, Thành Phố Hồ Chí Minh",
      url: "",
    },
    ...
    { id: 3, title: "📧 Email: info.viemall@gmail.com", url: "" },
    {
      id: 4,
      title:
        "✎ Mã số doanh nghiệp: 0318160848 do sở kế hoạch và đầu tư TP.Hồ Chí Minh cấp lần đầu ngày 13/11/2023",
      url: "",
    },
  ...
  return (
    <div
      className={
        _path === ROUTER?.ORDER_SUCCESS
          ? "hidden"
          : `bg-[#f5ebde] p-[10px] !w-full ${
              _path === ROUTER?.CONTACT ? "absolute bottom-0" : ""
            }`
      }
    >
      <div className="flex flex-row items-start justify-around m-[10px]">
        ...
        <div className="flex flex-col items-start">
          <h3 className="footer__policy--title footer--title font-medium text-[20px]">
            Chính sách
          </h3>
          <ul className="footer__policy__list flex items-start flex-col mt-[20px] gap-2">
            {lstServices?.map((item) => (
              <Link
                ...
                onClick={() => handleNextPageServices(item?.url)}
              >
                {item?.title}
              </Link>
            ))}
        ...
        <div className="flex flex-col items-start">
          <h3 className="footer__policy--title footer--title font-medium text-[20px]">
            CÔNG TY TNHH INCA GROUP
          </h3>
          <ul className="footer__policy__list flex items-start flex-col mt-[20px] gap-2">
            {lstContact?.map((item) => (
              <li
                key={item?.id}
                className="footer__information__contact--list-item footer--content !text-black"
              >
                {item?.title}
              </li>
            ))}
